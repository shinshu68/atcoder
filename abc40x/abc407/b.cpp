...
int main() {
	...
	for(int i = 1; i <= 6; i++) {
		for(int j = 1; j <= 6; j++) {
			if(i + j >= x) {
				cntX.push_back(make_pair(i, j));
			}
			if(abs(i - j) >= y) {
				cntY.push_back(make_pair(i, j));
			}
		}
	}
	set<pair<int, int>> cnt;
	rep(i, cntX.size()) {
		cnt.insert(make_pair(cntX[i].first, cntX[i].second));
	}
	rep(i, cntY.size()) {
		cnt.insert(make_pair(cntY[i].first, cntY[i].second));
	}
	cout << fixed << setprecision(10);
	cout << (double)cnt.size() / 36 << endl;
